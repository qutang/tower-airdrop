...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
	...
	public static BoxObject getMainBox() {
		for (BaseObject obj : actors) {
			if (obj.getObjectType() == ObjectType.MAIN_BOX) {
	...
	public static TriangleObject getParachute() {
		for (BaseObject obj : actors) {
			if (obj.getObjectType() == ObjectType.PARACHUTE) {
	...
	public static BaseObject getObjectFromBody(Body body) {
		for (BaseObject obj : actors) {
			try {
				if (obj.getPhysicalBody().equals(body)) {
					return obj;
				}
			} catch (Exception e) {
				continue;
	...
	public void render(Canvas canvas) {
		...
		synchronized (GameView.actors) {
			...
			while (it.hasNext()) {
				...
				}else if(obj.getObjectType() == ObjectType.BOX){
