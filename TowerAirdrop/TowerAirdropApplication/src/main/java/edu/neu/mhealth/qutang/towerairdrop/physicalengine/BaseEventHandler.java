...
import edu.neu.mhealth.qutang.towerairdrop.activities.GameSettingActivity;
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.BackgroundObject;
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.BaseObject;
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.BaseObject.ObjectType;
import edu.neu.mhealth.qutang.towerairdrop.Constants.BUTTON_TYPE;
import edu.neu.mhealth.qutang.towerairdrop.Constants.DIALOG_FROM;
import edu.neu.mhealth.qutang.towerairdrop.Constants.EMERGENCY_TYPE;
...
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.BaseObjectPool;
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.BoxObject;
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.ButtonObject;
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.ProgressObject;
import edu.neu.mhealth.qutang.towerairdrop.physicalengine.objects.TriangleObject;
...
public class BaseEventHandler extends Handler implements OnClickListener,
	...
	public void dispatchMessage(Message msg) {
		GAME_EVENTS event = Constants.GAME_EVENTS.values()[msg.what];
		switch (event) {
		case HANDLER_TEST:
			break;
		case BEING_SHAKING:
			...
			if (Flags.getFlags().shakeProgress * Flags.getFlags().totalMove < Flags
				...
				GameMusicPlayer.stopSound(mGameView.getContext());
			}
		...
		case PAUSE_SHAKING:
			...
			if (Flags.getFlags().shakeProgress * Flags.getFlags().totalMove > 0) {
				Flags.getFlags().shakeProgress -= Flags.getFlags().shakeSensitivity
						/ Flags.getFlags().totalMove;
				GameMusicPlayer.playSound(mGameView.getContext(), R.raw.alarm);
			}
			Flags.getFlags().shakeCounter--;
			break;
		case INIT_SCENE:
		...
		case SHOW_TOAST:
			...
			break;
		case BOX_COLLIDE:
			GameMusicPlayer.playSound(mGameView.getContext(), R.raw.collision);
			offerVibration();
			// update score
...
//						Gravity.TOP, 3000);
	...
	private void onSceneEnding() {
		...
		case Constants.TILTING_SCENARIO:
			...
			if (actor1.getPhysicalBody().getLinearVelocity().y == 0
...
//				Flags.getFlags().currentLevel++;
	...
	private void onGameRunning() {
		...
		case Constants.SHAKING_SCENARIO:
			...
			// generate emergency
			if (!Flags.getFlags().instructionShakeMode || !GameSettingActivity.getInstruction(mGameView.getContext())) {
			...
			break;
		case Constants.TILTING_SCENARIO:
			if (!Flags.getFlags().instructionTitleMode || !GameSettingActivity.getInstruction(mGameView.getContext())) {
...
//		}
	...
	private void onActionReady() {
		...
		case Constants.SHAKING_SCENARIO:
			...
			if (!GameSettingActivity.getInstruction(mGameView.getContext())) {
	...
	public void offerVibration(int length) {
		if (GameSettingActivity.getVibration(mGameView.getContext())) {
...
//						Flags.getFlags().currentLevel++;
			...
			ok_btn.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					...
					if(Constants.SHAREDPREFERENCES.getBoolean("changeInstructionSet", true)){
						...
						edit.commit();
						GameSettingActivity.disableInstruction(mGameView.getContext());
					}
				}
			});
			dialog.show();
	...
	private BoxObject generateNextLandscape(boolean randomVelocity,
		...
		if (landscape.getPhysicalBody() == null) {
	...
	private void generateNextObstacle(boolean randomVelocity) {
		...
		if (obstacle.getPhysicalBody() == null) {
	...
	private void makeMainBoxShaking() {
		...
		if (actor1.getPhysicalBody().m_linearVelocity.x == 0) {
			shake_speed.x = -shake_speed.x;
			actor1.setVelocity(shake_speed);
			boxShakeTimer = System.currentTimeMillis();
		} else if (actor1.getPhysicalBody().m_linearVelocity.x < 0
	...
	private void preventParachuteOverTurn() {
		if (Math.abs(actor2.getRotation()) > Constants.MAX_TILT_ANGLE) {
			actor1.getPhysicalBody().setAngularVelocity(0.0f);
			actor2.getPhysicalBody().setAngularVelocity(0.0f);
		}
	}
	...
	private void preventTiltingTooFast() {
		...
		if (direction_flag == Constants.TILT_LEFT) {
			if (actor2.getPhysicalBody().getLinearVelocity().x > -max_speed
	...
	private void destroyObjects() {
		...
		while (it.hasNext()) {
			...
			if (obj.getObjectType() == ObjectType.LANDSCAPE) {
				if (obj.getPosition().y < 0) {
					obj.destroySelf();
				}
			} else if (obj.getObjectType() == ObjectType.OBSTACLE) {
				if (obj.inContact) {
					Flags.getFlags().num_obstacles--;
					obj.destroySelf();
				}
			...
			} else if (obj.getObjectType() == ObjectType.PARACHUTE) {
	...
	public void beginContact(Contact contact) { // two object begin to collide
		...
		if (bodyA.getUserData() == ObjectType.MAIN_BOX) {
			...
			if (bodyB.getUserData() == ObjectType.BOX) { // stacked onto tower
				...
				if (!GameMusicPlayer.isPlayingSound()) {
					GameMusicPlayer.stopSound(mGameView.getContext());
					GameMusicPlayer.playSound(mGameView.getContext(), R.raw.box_down);
				}
			}
			...
			if (bodyB.getUserData() == ObjectType.OBSTACLE
					&& (Flags.getFlags().instructionTitleMode == false || !GameSettingActivity.getInstruction(mGameView.getContext()))
					&& Flags.getFlags().currentScore >= 0) {
